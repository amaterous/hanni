...
function makeOAuthCfg(overrides: Partial<NonNullable<HanniConfig["openai"]>["oauth"]> = {}): NonNullable<HanniConfig["openai"]>["oauth"] {
  return {
    clientId: "client_123",
...
describe("applyCodexDefaults", () => {
  it("fills in defaults when fields are empty", () => {
    const cfg = { clientId: "", redirectUri: "" } as NonNullable<HanniConfig["openai"]>["oauth"];
