...
import type { BroConfig, RepositoryConfig, SlackWorkspaceConfig } from "../types";
        ...
        for (const file of imageFiles) {
          try {
            const res = await fetch(file.url_private, {
              headers: { Authorization: `Bearer ${wsConfig.botToken}` },
            });
            if (res.ok) {
              ...
              log.info(`Downloaded image: ${file.name} (${buffer.length} bytes)`);
            }
          } catch (err) {
...
async function handleMention(
 ...
 try {
  ...
  const screenshotMatch = rawText.match(/(?:スクショ|screenshot|スクリーンショット)/i);
  const urlMatch = textForUrlMatch.match(/(https?:\/\/[^\s]+|[\w.-]+\.(?:site|com|net|org|dev|app|io)[^\s]*)/i);
  if (screenshotMatch) {
...
URLが特定できない場合は __URL__UNKNOWN__URL__ と出力して。`,
  ...
  try {
    ...
    if (result.resultText) {
      ...
      const truncated = formatted.length > 40000
        ? formatted.slice(0, 40000) + "..."
        : formatted;
...
const GITHUB_OWNER = "yuppppppppppqi";
...
async function createNewRepo(
  ...
  const proc = Bun.spawn(
    ["gh", "repo", "create", `${GITHUB_OWNER}/${repoName}`, "--private", "--clone=false", "--add-readme"],
  ...
  if (code !== 0) {
    if (stderr.includes("already exists")) {
      log.info(`GitHub repo already exists: ${GITHUB_OWNER}/${repoName}`);
    } else {
      throw new Error(`gh repo create failed: ${stderr.trim()}`);
    }
  } else {
    log.info(`GitHub repo created: ${GITHUB_OWNER}/${repoName}`);
  }

  return {
    name: repoName,
    github: `${GITHUB_OWNER}/${repoName}`,
    baseBranch: "main",
    linearWorkspaceId,
    projectKeys: [],
  };
